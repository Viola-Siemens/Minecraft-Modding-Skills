...
* 对外部可见的持久化格式（如 `SavedData`）需版本化（`data_version` 字段），并在读取时支持向后/向前兼容。示例：
...
* 不要直接序列化复杂对象为字符串，如果提供了 Codec，请务必使用；优先使用 `CompoundTag`、`ListTag` 等原生格式，并提供明确的读写方法（`serializeNBT()`、`deserializeNBT(CompoundNBT)`），且这些方法应有单元测试覆盖。
